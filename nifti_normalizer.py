...
def run_normalize(volume: np.ndarray, p_low: float = 0.5, p_high: float = 99.5) -> np.ndarray:
    arr = np.nan_to_num(volume.astype(np.float32), nan=0.0, posinf=0.0, neginf=0.0)
    ...
    return (arr - lo) / (hi - lo) * 2.0 - 1.0  # directly to [-1, 1]

if __name__ == "__main__":
    input_dir = Path("Dataset/training-t1-t2")
    output_dir = Path("Dataset/training-t1-t2-preprocessed")
    ...
    for i, nifti_path in enumerate(nifti_files):
        ...
        try:
            ...
            volume_norm = run_normalize(volume)

            out_path.parent.mkdir(parents=True, exist_ok=True)
            save_nifti(volume_norm, ref_img, str(out_path))
